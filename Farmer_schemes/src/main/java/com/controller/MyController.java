...
public class MyController {
	...
	/*  @RequestMapping(value = "/login", method = RequestMethod.GET)
	  ...
	  public ModelAndView loginProcess(HttpServletRequest request, HttpServletResponse response,   @ModelAttribute Farmer ifarmer) {
	    ...
	    Farmer farmer = farmerService.validatefarmer(ifarmer);
	    if (farmer != null) {
	      mav = new ModelAndView("welcome");
	      mav.addObject("fullname", farmer.getFull_name());
	      //session manage
	      HttpSession session= request.getSession();
	      session.setAttribute("farmername", ifarmer.getFull_name());
	    } else {
	      mav = new ModelAndView("login");
	      mav.addObject("message", "farmername or Password is wrong!!");
	    }
	    return mav;
	  }
